...
import { Utensils, Sparkles, Bookmark, X, Plus, Users, User, Calendar, Download, Upload, Info, ChevronDown, ChevronUp, RefreshCw, Clock, MapPin, CalendarIcon, Edit2, Settings2, Trash2, Link2, Calculator, StickyNote, Menu, Check, PowerOff, Camera, GripHorizontal } from 'lucide-react';
...
export default React.memo(function SideAccordionPanel({
  ...
  // 🔑 [신규] 예약 종료 패널이 열릴 때, 아직 아무것도 안 입력했으면 오늘 날짜로 기본값 채워둠
  useEffect(() => {
    if (activeSidePanel.includes('shutdown') && !shutdownTimeInput && !scheduledShutdownAt) {
      const now = new Date();
      const pad = (n) => String(n).padStart(2, '0');
      // 🔑 datetime-local input 형식: YYYY-MM-DDTHH:mm
      setShutdownTimeInput(`${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}T${pad(now.getHours())}:${pad(now.getMinutes())}`);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeSidePanel]);
  ...
  // 🔑 [수정] 프리셋 클릭 시 바로 예약하지 않고, "예약하기" 입력창에 시각만 채워넣음
  // (오늘 그 시각이 이미 지났으면 내일로 자동 계산)
  const handleUsePreset = (preset) => {
    ...
    const pad = (n) => String(n).padStart(2, '0');
    setShutdownTimeInput(`${target.getFullYear()}-${pad(target.getMonth() + 1)}-${pad(target.getDate())}T${pad(target.getHours())}:${pad(target.getMinutes())}`);
  };
                ...
                <>
                  ...
                  <div className="bg-[#F7F7F5] border border-[#E9E9E6] rounded-lg p-3 space-y-2">
                    ...
                    <button
                      type="button"
                      onClick={async () => {
                        if (!shutdownTimeInput || isSchedulingShutdown) return;
                        setIsSchedulingShutdown(true);
                        try {
                          await handleScheduleShutdown(new Date(shutdownTimeInput).toISOString());
                        } finally {
                          setIsSchedulingShutdown(false);
                        }
                      }}
                      disabled={!shutdownTimeInput || isSchedulingShutdown}
                      className="w-full py-1.5 bg-slate-700 hover:bg-slate-800 disabled:bg-gray-300 text-white rounded text-xs font-bold flex items-center justify-center gap-1.5"
                    >
                      {isSchedulingShutdown ? (
                        <>
                  ...
                  {/* 🔑 [수정] 자주 쓰는 예약 — 예약하기 영역 아래, 같은 폭의 카드 목록 (이름+시간 / 오른쪽 사용 버튼) */}
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                    ...
                    {shutdownPresets.length > 0 && (
                      <div className="space-y-1.5">
                        {shutdownPresets.map((preset) => (
                          <div key={preset.id} className="group relative flex items-center justify-between gap-2 bg-[#F7F7F5] border border-[#E9E9E6] rounded-lg px-3 py-2">
                            <div className="flex items-center gap-2 min-w-0">
                              <span className="text-xs font-bold text-gray-700 truncate">{preset.label}</span>
                              <span className="text-xs font-semibold text-gray-400 shrink-0">{String(preset.hour).padStart(2, '0')}:{String(preset.minute).padStart(2, '0')}</span>
                            </div>
                            <button type="button" onClick={() => handleUsePreset(preset)} className="px-2.5 py-1 bg-slate-700 hover:bg-slate-800 text-white rounded text-[11px] font-bold shrink-0">
                              사용
                            </button>
                            {/* 🔑 [수정] 카드 오른쪽 위에 겹쳐서 배치 — 평소엔 공간을 전혀 차지하지 않다가 호버 시에만 나타남 */}
                            <button
                              type="button"
                              onClick={() => handleDeletePreset(preset.id)}
                              className="hidden group-hover:flex items-center justify-center absolute -top-1.5 -right-1.5 w-4 h-4 bg-white border border-gray-300 rounded-full text-gray-400 hover:text-rose-600 hover:border-rose-300 shadow-xs"
                              title="삭제"
                            >
